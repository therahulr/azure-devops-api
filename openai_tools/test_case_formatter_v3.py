...
class TestCaseProcessor:
    def __init__(self, api_key=None, model="gpt-4.1", ado_area_path="Inficore", ado_iteration_path="Inficore\\Sprint 1", ado_assigned_to="rahulraj.cs26@gmail.com", processed_dir=None):
        ...
        # Directory for processed folders
    ...
    def prepare_test_case_prompt(self, test_case_data, steps_data, folder_name):
        ...
        You must strictly adhere to the following output JSON template schema and include the exact values for System.AreaPath, System.IterationPath, and System.AssignedTo:
        ...
        [
          {{
            ...
            "additional_fields": {{
              ...
              "System.AreaPath": "{self.ado_area_path}",
              "System.IterationPath": "{self.ado_iteration_path}",
              "System.AssignedTo": "{self.ado_assigned_to}"
            }}
          }}
        ]
        ```
        ...
        7. The System.AreaPath must be exactly "{self.ado_area_path}"
        8. The System.IterationPath must be exactly "{self.ado_iteration_path}"
        9. The System.AssignedTo must be exactly "{self.ado_assigned_to}"
        10. The JSON must be perfectly valid, with no syntax errors
    ...
    async def process_batch_async(self, batch, folder_paths):
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if isinstance(result, Exception):
                    ...
                    # Write the JSON output to the parent directory of the original folder
                    output_dir = os.path.dirname(folder_path)
                    output_file = os.path.join(output_dir, f"{folder_name.lower()}.json")
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump(result, f, indent=2)
                    
                    success_logger.info(f"Successfully processed {folder_name}")
                    
                    # Move the folder to the processed directory if specified
                    if self.processed_dir and folder_path:
                        # Create the processed directory if it doesn't exist
                        os.makedirs(self.processed_dir, exist_ok=True)
                        
                        # Move the folder
                        target_path = os.path.join(self.processed_dir, folder_name)
                        if os.path.exists(target_path):
                            # If target already exists, append a timestamp
                            timestamp = int(time.time())
                            target_path = os.path.join(self.processed_dir, f"{folder_name}_{timestamp}")
                        
                        shutil.move(folder_path, target_path)
                        logging.info(f"Moved {folder_name} to {target_path}")
    ...
    async def _call_openai_api_async(self, messages, folder_name):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check and fix the ADO fields if they're not set correctly
                if isinstance(parsed_json, list) and len(parsed_json) > 0:
                    for item in parsed_json:
                        if "additional_fields" in item:
                            # Ensure ADO fields are present and correctly set
                            item["additional_fields"]["System.AreaPath"] = self.ado_area_path
                            item["additional_fields"]["System.IterationPath"] = self.ado_iteration_path
                            item["additional_fields"]["System.AssignedTo"] = self.ado_assigned_to
    ...
    async def process_multiple_folders(self, folders, max_batch_size=5, max_tokens_per_batch=30000):
        ...
        # Prepare all prompts
        prompts = []
        folder_paths = []  # Keep track of the original folder paths
        ...
        # We need to maintain the association between prompts and folder paths
        prompt_folder_pairs = list(zip(prompts, folder_paths))
        prompt_folder_pairs.sort(key=lambda x: x[0]["token_count"])
        
        # Unzip the sorted pairs
        if prompt_folder_pairs:
            prompts, folder_paths = zip(*prompt_folder_pairs)
        else:
            prompts, folder_paths = [], []
        
        # Create batches based on token count
        current_batch_folder_paths = []
        for i, prompt in enumerate(prompts):
            ...
            if len(current_batch) >= max_batch_size or current_batch_tokens + prompt["token_count"] > max_tokens_per_batch:
                if current_batch:  # Only add non-empty batches
                    batches.append((current_batch, current_batch_folder_paths))
                current_batch = [prompt]
                current_batch_tokens = prompt["token_count"]
                current_batch_folder_paths = [folder_paths[i]]
            else:
                current_batch.append(prompt)
                current_batch_tokens += prompt["token_count"]
                current_batch_folder_paths.append(folder_paths[i])
        ...
        for i, (batch, batch_folder_paths) in enumerate(batches):
            logging.info(f"Processing batch {i+1}/{len(batches)} with {len(batch)} test cases")
            
            successful, failed = await self.process_batch_async(batch, batch_folder_paths)
...
def parse_args():
    ...
    parser.add_argument('--area-path', default='Inficore', help='Azure DevOps Area Path')
    parser.add_argument('--iteration-path', default='Inficore\\Sprint 1', help='Azure DevOps Iteration Path')
    parser.add_argument('--assigned-to', default='rahulraj.cs26@gmail.com', help='Azure DevOps Assigned To')
...
async def main_async():
    ...
    processed_dir = os.path.join(os.path.dirname(main_folder), "processed")
